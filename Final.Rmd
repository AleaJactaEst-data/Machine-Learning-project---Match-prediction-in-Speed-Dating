...
df_mod = resu$df_mod;train_data = resu$dapp
...
#tune_grid <- expand.grid(C=c(0.001,0.01,0.1,1), sigma = 1)

#mod.svm <- caret::train(match ~., data = dapp, method = "svmRadial",trControl=ctrl,tuneGrid = tune_grid,tuneLength = 10,preProcess = c("scale", "YeoJohnson"), metric = "F1")


#mod.svm <- svm(match~.,data=dapp,kernel="polynomial", cost=1)
#mod.svm <- svm(match~.,data=dapp,kernel="linear", cost=100)
tune.out <- tune(svm,match~.,data=dapp,kernel="polynomial",ranges=list(cost=c(0.1,1,10),gamma=0.1))
summary(tune.out)
...
```{r}
TrueNeg = caret::confusionMatrix(data=pred.svm,reference=dtest$match,positive="1")$table[1,1]
FalseNeg = caret::confusionMatrix(data=pred.svm,reference=dtest$match,positive="1")$table[1,2]
TruePos = caret::confusionMatrix(data=pred.svm,reference=dtest$match,positive="1")$table[2,2]
FalsePos = caret::confusionMatrix(data=pred.svm,reference=dtest$match,positive="1")$table[2,1]
...
Cepedant on passe d'un F1 score de 0.01 à 0.37 : cela est lié au fait que le taux de Vrai Positif est beaucoup plus important. Avec un jeu de données plus équilibré, le modèle est "mieux entrainé" et prédit mieux les 1.
...
df_mod = resu$df_mod;train_data = resu$dapp;test_data = resu$dtest
# base oversampling
dapp <- ovun.sample(match ~ ., data = train_data, method = "over",N = 2*(nrow(df_mod) - sum(as.numeric(df_mod$match)-1)))$data
table(dapp$match)
dapp = data.frame(lapply(dapp, function(x) as.numeric(as.character(x))))
ncol = ncol(dapp) - 1 
...
score =  (f1_rose*predict_proba(model_rose,xtest_normal)
                        ...
                        + f1_poids*predict_proba(model_poids,xtest_normal))/(f1_rose+f1_over_1+f1_poids)
