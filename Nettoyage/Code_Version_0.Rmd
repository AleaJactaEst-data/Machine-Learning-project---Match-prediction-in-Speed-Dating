...
```{r}
df = read.csv("C:/Users/xgodw/Desktop/4GM/Apprentissage_statistique/Projet - Speed Dating/Speed Dating Data.csv")
```
...
```{r}
df_couple_id = df2[,c('iid','pid', 'match', 'int_corr')] # samerace a garder ? non on le recrée après
...
varSignificatifs = c("age_moy" , "age_ect" , "imprace_moy" , "dining_moy" ,
    ...
    "go_out_1","go_out_3" , "go_out_6" , "career_c_1" , "career_c_7" , 
...
df_mod = df_couple2[,which(names(df_couple_2) %in% varSignificatifs)]
n = nrow(df_couple2)
set.seed(1234)
...
sum(pred_best != dtest$match)/nrow(dtest) # 21%
sum(pred_best != dtest$match & dtest$match == 1)/sum(as.numeric(dtest$match)-1) #♠ 0.67

```
```{r}
remove(n, perm,dapp, dtest)
```
################################################ TEST SVM  #################################################""
...
df_mod = df_couple2[,which(names(df_couple2) %in% varSignificatifs_2)]
#df_mod = df_mod[,c(1,2:160)]
...
df_mod = df_couple2[,-which(names(df_couple2) %in% varListe)]
df_mod = df_couple2[,which(names(df_couple2) %in% c('match','int_corr'))]
#df_mod = df_mod[,c(1,2:160)]
...
mod.svm <- svm(match~distance+same_field_cd+shar2_avg_moy+amb4_avg_moy+fun1_avg_moy+sinc4_avg_moy+age_ect,data=dapp,kernel="linear", cost=100000)
...
sum(pred.svm != dtest$match)/nrow(dtest) # 0.27
sum(pred.svm != dtest$match & dtest$match == 1)/sum(as.numeric(dtest$match)-1) # 0.69
```
...
sum(predSimple != dtest$match)/nrow(dtest) #0.22 / 0.26 / 0.3 / 0.23
sum(predSimple != dtest$match & dtest$match == 1)/sum(dtest$match) #0.77 / 0.76 / 0.57 / 0.7
```
