...
# creation d'une variable distance
...
df_couple2 = df_couple2[,-which(names(df_couple2) %in% c("gender.x", "gender.y", "go_out_6", "id" ,"iid", "pid"))]
...
varRandom_1 = c(rownames(temp)[which(temp > mean(temp))], "match")
varRandom_2 = c(rownames(temp)[which(temp > mean(temp) + sd(temp))], "match")
```
...
varBoost_1 = c(as.character(res$var[which(res$rel.inf > mean(res$rel.inf))]), "match")
varBoost_2 = c(as.character(res$var[which(res$rel.inf > mean(res$rel.inf) + sd(res$rel.inf))]), "match")
```
...
res = creationDesData(df_couple2, varSignificatifs = varBoost_2, varSupp =  varSupp, noFactor = FALSE)
df_mod = res$df_mod
dapp = res$dapp
dtest = res$dtest
```
...
print(caret::confusionMatrix(data=factor(pred_best),reference=dtest$match,positive="1")) # res 66% acc et sensitivity 40%
```
...
ctrl <- trainControl(method = "cv", savePredictions = TRUE, summaryFunction = f1)

tune_grid <- expand.grid(C=c(2000, 5000, 10000, 100000), sigma = seq(0.01,0.2,0.03))

mod.svm_ <- caret::train(match ~., data = dapp, method = "svmRadial",trControl=ctrl,tuneGrid = tune_grid,preProcess = c("scale", "YeoJohnson"), metric = "F1")
...
pred.svm_ = predict(mod.svm_, newdata = dtest)
print(caret::confusionMatrix(data=pred.svm_,reference=dtest$match,positive="1"))
...
```{r echo = TRUE, eval=FALSE, include=FALSE, results = 'hide'}
# resultat pour gamma = 
res = optimisationNeuroneDeeplearning(4.8, 5.2, 0.1, 800, xtrain, xtest,ytrain, ytest);
...
focal_loss=function(y_true, y_pred){
  gamma = 5.1 # 3.8 a 7.5 fonctionne > test - amelioration surechantilonage
  print("gamma")
...
# compilationf, apprentissage et prédiction
```{r,  results = 'hide', message=FALSE, paged.print=FALSE, eval=FALSE, include=FALSE}
model %>% compile(
  loss = focal_loss,
