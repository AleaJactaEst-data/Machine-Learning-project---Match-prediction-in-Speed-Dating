...
plot(rf_fit_down)
# Testing
f1_eval3=best_f1(rf_fit_down,test_data)
...
df_toto = bind_rows(as.data.frame(f1_eval),as.data.frame(f1_eval2),as.data.frame(f1_eval3))
  ggplot(df_toto, aes(x=seuil,y=F1,color=methode))+geom_line()+ggtitle("Comparaison des algorithme xgboost")
...
Ici un vote à la majorité avec les meilleurs IA : 2 suréchantillonage et 1 Rose. On obtient un F1_score de 0.5732 ce qui est sensiblement pareil que le F1_score de "model_poids" seul.
```{r}
predSimple = as.integer(as.integer(predict_proba(model_rose,xtest_normal)>0.5)
                        ...
                        +as.integer(predict_proba(model_poids,xtest)>0.5)
                        >1.5)
```
```{r,echo=TRUE}
print(caret::confusionMatrix(data=factor(predSimple),reference=factor(ytest_normal),positive="1",mode = "prec_recall"))
...
score =  (f1_rose*predict_proba(model_rose,xtest_normal)
                        ...
                        + f1_poids*predict_proba(model_poids,xtest))/(f1_rose+f1_over_1+f1_poids)
