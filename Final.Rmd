...
Dans un premier temps il est nécéssaire de nettoyer notre jeu de données et de le préparer pour répondre à notre problématique
...
L'étude a été faite sur plusieur vagues et durant la notation des attribut (sincérité, intelligence...), deux systemes de notation ont été utilisés pour un même sujet. C'est pourquoi, afin d'homogéniser le systeme de points ainsi que de répondre à notre objectif, nous avons harmoniser le systeme de points puis effectuer une moyenne sur toutes les vagues pour chaque attribut concernant le meme sujet (perception de soie, attentes de son partenaire..)  
```{r}
...
## Aggregation
Certaines variables qualitatives disposait d'un grands nombre de madalité (parfois 17) c'est pourquoi pour certaines d'entres elles, nous avons créé une nouvelles variables qui les reprends en aggrégeant certaines de leurs modalité.
```{r}
...
Cette fonctions va nous permettre de créer notre base couple finale en faisant en sorte qu'un couple n'apparaissent qu'une fois et que les informations des deux partenaire soient présents pour chaque ligne. Les variable se terinant par .x désignent celles relatives aux femmes et par .y celles relaives aux hommes
```{r}
...
Enfin nous avons définit un fonction qui fait appelles à celles dont nous venons d'évoquer plus haut.
```{r}
...
# Feature engineering
Une fois que nous avons notre base couple, nous avons fait quelques création de nouvelles variables.

## Variables quantitatives
Dans cette partie, nous créeons plusieurs variables afin de mettre en lien les variables des hommes et des femmes comme des variables disatnces qui calcule la disatnce/norme entre les activié des hommes et des femmes.
```{r}
...
Dans cette partie,nous créeon principalement des variables indicatrice et de comptage.
```{r}
...
L'approche SVM (Support Vector Machines) cherche le meilleur hyperplan en terme de séparation des données. Dans notre cas, avec plus que 100 variables explicatives, il est très probable que les données seraient séparables et nous allons des modèles SVM non-séparable. 
...
Pour corriger cet erreur, nous allons réeschantilloner la base avec la méthode Rose permetttant de rééquilibrer les classes. Cette méthode crée une base de donnée équilibré par des méthodes de bootstrap et d'individus artificielles. Elle donne souvent de bons résulats.
```{r}
dtest = test_data
# base rose
...
```
Nous obtenons un mieux modèle. Le meilleur valeur pour C est 0.1 et pour ce modèle, 66.3% des rencontres sont correctement prédites et 57% des rencontres match et 68,19% des rencontres non-match sont correctement prédites. Nous calculons le F1 score pour mesurer l'équilibre entre la précision et le rappel. 
```{r}
...
Ensuite on essaie le modèle SVM radial. On cherche à optimiser les paramatres de cout et de gamma.
```{r}
...
Le modèle également mieux les 1, le rappel est donc meilleur que sans travail sur l'équilibrage de jeu de données.
...
# XGBOOST
Dans cette partie on va s'intérésser à une méthode de modélisatioin utilisant l'aggrégation par boosting. Pour cela on utiliser comme modèle de base l'arbre de décision. Les hyperparamètres à optimiser sont :

** le nombre d'itération du boosting
** la profondeur maximale des arbres 
** le pas de descente de gradient
** le pourcentage de variable qu'on garde pour la construton de chaque arbre
...
rappel_prec<-function(model,test_data,min=0.01,max=0.99,step=0.001){
  seuil=seq(min,max,step)
  resu=sapply(seuil,function(x){
    #print(x)
    # Testing
    final <- data.frame(actual = test_data$match,predict(model, newdata = test_data, type = "prob"))
    final$predict <- factor(ifelse(final$X0 > x, "0", "1"),levels = c("0","1"))
...
courbe_rappel_prec(rf_fit_down,test_data)
...
Nous avons vu pleins de méthodes pour contrer le déséquilibre des classes et les méthodes rééchantillonages paraissent vraiment adéquates. Nous allons maintenant essayer de combiner les meilleurs modèles soient les modèles avec IA et XGBoost afin que les avantages des uns rattrapent les inconvénients des autres. Pour cela, nous allons effectué un vote à la majorité oen pondérant ces résultats par rapport aux meilleurs F1_score. Les meilleurs seuils seront aussi utilisé à chaque fois pour maximiser le F1_score.
...
```{r}
score = (f1_rose * as.integer(predict_proba(model_rose,xtest_normal)>meilleur_seuil_rose)
                        +f1_over_1 * as.integer(predict_proba(model_over_1,xtest_normal)>meilleur_seuil_over_1)
                        +f1_poids * as.integer(predict_proba(model_poids,xtest)>meilleur_seuil_poids)
+ f1_eval$F1[which.max(f1_eval$F1)] *
  as.integer(data.frame(actual = test_data$match,predict(model, newdata = test_data, type = "prob"))$X0 > f1_eval$seuil[which.max(f1_eval3$F1)])
+ f1_eval2$F1[which.max(f1_eval2$F1)] * 
  as.integer(data.frame(actual = test_data$match,predict(model, newdata = test_data, type = "prob"))$X0 > f1_eval2$seuil[which.max(f1_eval3$F1)])
+ f1_eval3$F1[which.max(f1_eval3$F1)] * 
  as.integer(data.frame(actual = test_data$match,predict(model, newdata = test_data, type = "prob"))$X0 > f1_eval3$seuil[which.max(f1_eval3$F1)]))
  /(f1_rose+f1_over_1+f1_poids+f1_eval$F1[which.max(f1_eval$F1)]+f1_eval2$F1[which.max(f1_eval2$F1)]+f1_eval3$F1[which.max(f1_eval3$F1)])
...
On obtient un super modèle avec un très bon F1 score et une bonne exactitude. Il est le meilleur modèle à ce jour. Ceci n'est qu'une ébauche qu'il serait intéressant d'approfondir.
